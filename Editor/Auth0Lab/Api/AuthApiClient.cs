...
namespace Auth0.Api
{
    ...
    public class AuthApiClient : AuthenticationApiClient 
    {
        ...
        public async Task<AccessTokenResponse> GetAccessTokenPkceAsync(string clientId, string codeVerifier, string code, string redirectUri, int retryInterval, CancellationToken cancellationToken = default, Action<string> onFailure = null)
        {
            AccessTokenResponse response = null;
            ErrorApiException apiError;
            ...
            do
            {
                ...
                catch (ErrorApiException ex)
                {
                    ...
                    if(retryTimes >= 3)
                    {
                        Exception exception = new Exception($"Failed to authenticate! Try relaunching. (ERROR=0). {ex}");
                        Debug.LogException(exception);
                        onFailure?.Invoke("Failed to authenticate! Try relaunching. (ERROR=0)");
                    }
                }
            } while ((apiError != null && apiError.ApiError.ErrorCode != "authorization_pending") || response == null);
